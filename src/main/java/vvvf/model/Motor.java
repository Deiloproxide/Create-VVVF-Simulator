...
public class Motor {
    ...
    private double[] derivatives(double ids, double iqs, double ird, double irq, double wr, double ws, double vds, double vqs) {
        ...
        double[] solD = solve2x2(
                specification.ls, specification.lm,
                specification.lm, specification.lr,
                vds - specification.rs * ids + ws * psiSq,
                -specification.rr * ird + (ws - wr) * psiRq
        );
        double dids = solD[0];
        double dird = solD[1];

        double[] solQ = solve2x2(
                specification.ls, specification.lm,
                specification.lm, specification.lr,
                vqs - specification.rs * iqs - ws * psiSd,
                -specification.rr * irq - (ws - wr) * psiRd
        );
        double diqs = solQ[0];
        double dirq = solQ[1];
    ...
    private double[] solve2x2(double a11, double a12, double a21, double a22, double b1, double b2) {
        ...
        if (Math.abs(det) < 1e-12) {
            det = Math.signum(det) * 1e-12 + 1e-12;
        }
    ...
    private double frictionTorque(double wr, double teMinusTl) {
        ...
        if (Math.abs(wr) < epsW && Math.abs(teMinusTl) < specification.fs) {
            return teMinusTl;
        }
