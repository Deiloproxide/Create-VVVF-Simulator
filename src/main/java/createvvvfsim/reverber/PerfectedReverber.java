...
public class PerfectedReverber extends Reverber{
    private static final int buffer_size=SpecConfig.buffer_size.get();
    private static final int tail_size=SpecConfig.tail_size.get();
    ...
    public EnvData getEnv(Level level,Player player,Vec3 train_pos){
        ...
        try{
            ...
            double reverb_sth=Instance.invokeStatic(Double.class,reverb_strength)/
                    Instance.invokeStatic(Double.class,reverb_denom);
            double weighted_sth=Instance.invokeStatic(Double.class,weighted_strength)/
                    Instance.invokeStatic(Double.class,reverb_denom);
            double leak=Instance.invokeStatic(Double.class,outdoor_leak)/
            ...
            double enclosure=Math.min(Math.max((indoors?1.0-leak:reverb_sth-leak)*bias,0.0),1.0);
            ...
            double overall=Math.min(Math.max(enclosure*distance_mul*room_mul*
                    (PerfectedConst.base_gain+reverb_sth*PerfectedConst.gain_mul)*
            ...
            double[] weights={early_weight,weighted_sth,late_weight,late_weight};
            for(int i=0;i<4;i++){
    ...
    public void handle(double[] mix_buffer){
        for(TrainData data:GlobalData.trains){
            ...
            EnvData current=data.current,target=data.target,step=data.step;
            Lowpass[] data_filters=data.filters;
            if(data.is_mute){
                EnvCalc.jumpStep(current,target);
                continue;
            }
            for(int i=0;i<buffer_size;i++){
                EnvCalc.addStep(current,step);
                double[] cutoffs=current.cutoffs;
                double alpha=Math.min(Math.max(current.cutoff,0.02),1.0);
                data_filters[4].set(alpha);
                mix_buffer[i]+=data_filters[4].process(train_buffer[i])*current.gain;
                for(int j=0;j<4;j++){
                    ...
                    data_filters[j].set(alpha);
                    tail_buffers[j][tail_ptr]+=data_filters[j].process(train_buffer[i]*current.gains[j]);
                }
            }
        }
        for(int i=0;i<buffer_size;i++){
