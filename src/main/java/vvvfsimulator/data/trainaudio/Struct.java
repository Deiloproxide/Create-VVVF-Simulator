...
public class Struct {
    ...
    private Struct(boolean applyDefaultLists) {
        ...
        if (applyDefaultLists) {
            applyOriginalDefaultListParameters();
        }
    }

    public Struct copy() {
        Struct copy = new Struct(false);
        for (HarmonicData h : gearSound) {
            copy.gearSound.add(h.copy());
        }
        for (HarmonicData h : harmonicSound) {
            copy.harmonicSound.add(h.copy());
        }
        for (SoundFilter f : filters) {
            copy.filters.add(f.copy());
        }
